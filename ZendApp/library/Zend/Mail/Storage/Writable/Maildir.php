...
{
    // : init maildir (+ constructor option create if not found)
    ...
    protected function _calculateQuota($forceRecalc = false) {
        ...
        if ($fh) {
            //  is there a safe way to keep the handle open for writing?
            fclose($fh);
        }
    ...
    protected function _addQuotaEntry($size, $count = 1) {
        if (!file_exists($this->_rootdir . 'maildirsize')) {
            // : should get file handler from _calculateQuota
        }
        $size = (int)$size;
        $count = (int)$count;
